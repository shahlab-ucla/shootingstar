function varargout = lineage_launcher_online(varargin)
%lineage_launcher_online M-file for lineage_launcher_online.fig
%      lineage_launcher_online, by itself, creates a new lineage_launcher_online or raises the existing
%      singleton*.
%
%      H = lineage_launcher_online returns the handle to a new lineage_launcher_online or the handle to
%      the existing singleton*.
%
%      lineage_launcher_online('Property','Value',...) creates a new lineage_launcher_online using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to lineage_launcher_online_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      lineage_launcher_online('CALLBACK') and lineage_launcher_online('CALLBACK',hObject,...) call the
%      local function named CALLBACK in lineage_launcher_online.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help lineage_launcher_online

% Last Modified by GUIDE v2.5 28-Apr-2015 10:11:52

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @lineage_launcher_online_OpeningFcn, ...
                   'gui_OutputFcn',  @lineage_launcher_online_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before lineage_launcher_online is made visible.
function lineage_launcher_online_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line outputname for lineage_launcher_online
handles.outputname = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes lineage_launcher_online wait for user response (see UIRESUME)
...
function varargout = lineage_launcher_online_OutputFcn(hObject, eventdata, handles)
...
TargetCell = get(handles.TargetCell,'String');

detect_track_driver_allmatlab_online(parameterfilenames{i},embryodir,handles.embryoname,[suffix,num2str(i)],outputdirectory,points,false,TargetCell);%,lineageparameterfile);
