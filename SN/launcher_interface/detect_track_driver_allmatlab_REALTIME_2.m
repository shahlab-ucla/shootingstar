...
function detect_track_driver_allmatlab_REALTIME_2(parameterfile,embryodir,embryonumber,suffix,outputdirectory,polygon_points,isred,TargetCell)
...
for init = 1:run_time %Force pipeline to run one image at a time
    ...
    if init>5
        fid = fopen('C:\Users\shahp2\Desktop\MicroPointTarget\TargetCell_2.txt','r');
        if fid ~= -1
            text = fscanf(fid,'%s');
            fclose(fid);
            ...
            for k = 1:NumTargets
                if ~strcmp(TargetCell,'none')
                    ...
                    if Exists
                        fid2 = fopen('C:\Users\shahp2\Desktop\MicroPointTarget\log_2.txt','a');
                        ...
                        if persistance(k) >= thresh(k) && killed(k) < 1  && used == 0
                            ...
                            fid = fopen('C:\Users\shahp2\Desktop\MicroPointTarget\coords_2.ini','w');
...
esequence = master_esequence;
%save('times2.mat','esequence','time_to_read','time_to_link_hard','total_time','tracking_time','segmentation_time','time_to_filter','TimeSpentMerging');
%Write out a backup
