...
function detect_track_driver_allmatlab_micromanager(parameterfile,embryodir,embryonumber,suffix,outputdirectory,polygon_points,isred,TargetCell,P)
...
for init = 1:run_time %Force pipeline to run one image at a time
    ...
    end_time = init;
    tseg = tic;
    processSequence_micromanager; %start segmentation for the timepoint as defined by init
    ...
    if init>5
        fid = fopen(strcat('C:\Ablation\TargetCell_',num2str(P),'.txt'),'r');
        if fid ~= -1
            text = fscanf(fid,'%s');
            fclose(fid);
            ...
            for k = 1:NumTargets
                if ~strcmp(TargetCell,'none')
                    ...
                    if Exists
                        fid2 = fopen('C:\Users\shahp2\Desktop\MicroPointTarget\log_1.txt','a');
                        ...
                        if persistance(k) >= thresh(k) && killed(k) < 1
                            ...
                            Y = Pos(2);
                            Z = Pos(3);
                            save(strcat('C:\Ablation\Coords_',num2str(P)','_',num2str(used),'.mat'),'P','X','Y','Z');
                            used=used+1;
                        end
                    else
...
esequence = master_esequence;
%save('times1.mat','mem','esequence','time_to_read','time_to_castrays','time_to_merge_nuclei','time_to_link_hard','total_time','tracking_time','segmentation_time','time_to_filter','TimeSpentMerging');
%Write out a backup
