...
function detect_track_driver_allmatlab_REALTIME_1(parameterfile,embryodir,embryonumber,suffix,outputdirectory,polygon_points,isred,TargetCell)
...
if(exist('SIMPLETIFF'))
    fprintf (file,'<splitChannelImage type="0"/>\n');
end
...
for init = 1:run_time %Force pipeline to run one image at a time
    ...
    %{
    if init>5
        fid = fopen('C:\Users\shahp2\Desktop\MicroPointTarget\TargetCell_1.txt','r');
        if fid ~= -1
            text = fscanf(fid,'%s');
            fclose(fid);
                ...
                if ~strcmp(TargetCell,'none')
                    ...
                    if Exists
                        ...
                        if persistance(k) >= thresh(k) && killed(k) < 1  && used == 0
                            ...
                            fid = fopen('C:\Users\shahp2\Desktop\MicroPointTarget\coords_1.ini','w');
                            fprintf(fid,strcat('[main]\r\nstatus=Ready\r\nX=',num2str(512-Pos(1)-3),'\r\nY=',num2str(Pos(2)),'\r\nZ=',num2str(Pos(3)),'\r\n'));
