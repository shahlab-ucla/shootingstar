function varargout = lineage_launcher_REALTIME_micromanager(varargin)
%lineage_launcher_REALTIME_micromanager M-file for lineage_launcher_REALTIME_micromanager.fig
%      lineage_launcher_REALTIME_micromanager, by itself, creates a new lineage_launcher_REALTIME_micromanager or raises the existing
%      singleton*.
%
%      H = lineage_launcher_REALTIME_micromanager returns the handle to a new lineage_launcher_REALTIME_micromanager or the handle to
%      the existing singleton*.
%
%      lineage_launcher_REALTIME_micromanager('Property','Value',...) creates a new lineage_launcher_REALTIME_micromanager using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to lineage_launcher_REALTIME_micromanager_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      lineage_launcher_REALTIME_micromanager('CALLBACK') and lineage_launcher_REALTIME_micromanager('CALLBACK',hObject,...) call the
%      local function named CALLBACK in lineage_launcher_REALTIME_micromanager.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help lineage_launcher_REALTIME_micromanager

% Last Modified by GUIDE v2.5 06-Mar-2020 11:25:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @lineage_launcher_REALTIME_micromanager_OpeningFcn, ...
                   'gui_OutputFcn',  @lineage_launcher_REALTIME_micromanager_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before lineage_launcher_REALTIME_micromanager is made visible.
function lineage_launcher_REALTIME_micromanager_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line outputname for lineage_launcher_REALTIME_micromanager
handles.outputname = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes lineage_launcher_REALTIME_micromanager wait for user response (see UIRESUME)
...
function varargout = lineage_launcher_REALTIME_micromanager_OutputFcn(hObject, eventdata, handles)
...
function runbutton_Callback(hObject, eventdata, handles,gui)
...
detect_track_driver_allmatlab_micromanager(parameterfilenames{i},embryodir,handles.embryoname,[suffix,num2str(i)],outputdirectory,points,false,'',gui);%,lineageparameterfile);
...
function imagebutton_Callback(hObject, eventdata, handles,gui)
% hObject    handle to imagebutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
acq = gui.getAcquisitionManager();
if ~acq.isAcquisitionRunning()
    errordlg("Start acquisition before loading image");
else
    display = gui.getDisplayManager();
    window = display.getCurrentWindow();
    data = window.getDatastore();
    DataManager = gui.getDataManager;
    builder = DataManager.getCoordsBuilder();
    test = data.getAnyImage();
    maxcoords = data.getMaxIndices();
    nplanes = maxcoords.getZ()+1;
    im = zeros(test.getHeight(),test.getWidth(),nplanes,'uint16');
    ...
    for i = 1:nplanes
        builder.channel(c).z(i).t(0).p(0);
        coords = builder.build();
        im = data.getImage(coords);
        im = reshape(im.getRawPixels(),[test.getHeight(),test.getWidth()])';
    end
end
