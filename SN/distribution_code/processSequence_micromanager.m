...
for example=1:length(tlist)
    ...
    if firsttimestep == 1
        ...
        previous=master_esequence{init-1};
    end
    ...
    if (MATLAB_STACK)
        ...
        if(newscope)
            ...
            if rednuclei
                while check == 1
                    if exist([embryodir,embryonumber,'_t',num2str(time),'.tif'], 'file') == 2
                        check = 0; 
                    end
                    pause(1)
                end
    ...
    for i=1:length(good)
       ...
       % bottoms1=[bottoms1;max(planes)];
        planes=esequence{good(i)}.finalpoints(:,3);
         ...
         parfor j=1:svol(3) %ignore maxima on bottom 2 planes, they're artifacts
