...
volsize=size(X);
tempcenteredxymax=diskSet.centeredxymax;
tempxydetdiameters=diskSet.xydetdiameters;
tempcenterindicies=nucleiSet.centerindicies;
tempxymaximavals=diskSet.xymaximavals;
tempxycoverage=diskSet.xycoverage;
parfor i=1:length(tempcenterindicies)
    diam=tempxydetdiameters(tempcenterindicies(i));
    centerval=tempcenteredxymax(tempcenterindicies(i),:);
    maximaval=tempxymaximavals(tempcenterindicies(i),:);
    resultval=assign_planes(centerval,diam,volsize,tempcenteredxymax,anisotropy);
    logodds{i}=calculateLogodds(resultval,centerval,maximaval,diam,tempxymaximavals,tempxydetdiameters,anisotropy,tempxycoverage);
    centers{i}=resultval;
end
...
while (~isempty(find(indicies2, 1))) 
...
for i=1:length(unclaimeddiams) 
 centers3{i}=assign_planes(unclaimedcentered3(i,:),unclaimeddiams(i),volsize,diskSet.centeredxymax,anisotropy);
